...
- N-004 — Connect one bounded asynchronous shallow-direct work-admission path from immutable sample plans to the publication gate. Work items must carry canonical key, method/version, exact request epoch, progress, and provenance; stale completion must be harmless, and no per-batch readback may be awaited by input or `requestAnimationFrame`.
- N-005 — Extend planning to an exact, versioned non-square viewport domain without deriving authority from CSS/presentation transforms; retain the bounded square planner as the stable fallback.
