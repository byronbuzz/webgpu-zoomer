...
- N-004 — Extend planning to an exact, versioned non-square viewport domain derived from explicit integer display dimensions, without consuming CSS/presentation transforms as numerical evidence; retain the bounded square planner as the stable fallback.
- N-005 — Build the first one-way presentation snapshot from accepted numerical samples plus explicit unresolved coverage, then composite it downstream without allowing it to become recurrence/reference input.
- N-006 — Prototype a bounded high-throughput browser reference/oracle path before increasing dense-grid iteration budgets; include time/memory/work limits that fail to `unresolved`, because iteration count alone did not bound exact-dyadic numerator growth.
- N-007 — Perturbation, reference transport, rebasing, series/BLA, compaction, and broader production scheduling remain separate promotion tasks with their own executable validity/error contracts.
- N-008 — Calibrate production interaction and convergence thresholds with longer representative trajectories after the asynchronous authoritative path exists; the current interpolation ceilings are smoke-test limits, not release performance claims.
